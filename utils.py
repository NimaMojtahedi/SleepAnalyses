...
def get_data(address, sf):
    ...
    print(
        f'data and hypno shapes in file {address} are: {data.shape}, {hypno.shape}')
...
def find_type2_locations(dictLists, pattern=[0, -2], cond1_min=100, cond1_max=10000, cond2_min=100, cond2_max=10000):
    ...
    # first element
    window_cond1 = [(myDict['epochDuration'] > cond1_min and myDict['epochDuration']
                     < cond1_max) for myDict in dictLists]

    # second element
    window_cond2 = [(myDict['epochDuration'] > cond2_min and myDict['epochDuration']
                     < cond2_max) for myDict in dictLists]
    ...
    locs = np.where((np.array(all_states[:-1]) == pattern[0]) &  # first element of pattern
                    # second element of the pattern
                    (np.array(all_states[1:]) == pattern[1]) &
                    # pattern comming from same mouse
                    (np.array(all_names[:-1] == np.array(all_names[1:]))) &
...
def find_type3_locations(dictLists, pattern=[-2, -3, -2], cond1_min=100, cond1_max=10000,
    ...
    # third element
    window_cond3 = [(myDict['epochDuration'] > cond3_min and myDict['epochDuration']
                     < cond3_max) for myDict in dictLists]
...
def find_type4_locations(dictLists, pattern=[0, -2, -3, -2], cond1_min=100, cond1_max=10000,
    ...
    temp_loc1 = np.array([i for i in range(len(dictLists))
                         if dictLists[i]['hypnoState'] == 0])
    ...
    for i in range(type3_locs.shape[0]):
        if i == 0:
            ...
            temp = []
            temp = temp_loc1[np.where((temp_loc1 < type3_locs[i, 0]) & (
                temp_loc1 > type3_locs[i-1, 2]))[0]]
        # pdb.set_trace()
...
def div_avg_firing(data, sf, n=3):
    ...
    avg = [avg_firing(data=data[spaces[i]:spaces[i+1], :], sf=sf)
           for i in range(n)]
    return avg
...
def div_avg_amplitude(data, n=3):
    spaces = np.linspace(0, data.shape[0], n+1, dtype=int)
    avg = [avg_amplitude(data=data[spaces[i]:spaces[i+1], :])
           for i in range(n)]
    return avg
...
def search_spindle(data_spindle, Dict, mouse_index):
    ...
    if temp.any():
        locations = np.squeeze(np.where(
            (temp[:, 1] >= Dict['startOfEpoch']) & (temp[:, 2] <= Dict['endOfEpoch'])))
        ...
        if locations.any():
            ...
            if selected_locations.ndim == 1:
                data = Dict['data'][(selected_locations[1] - Dict['startOfEpoch']): (
                    selected_locations[1] - Dict['startOfEpoch']) + selected_locations[3], :]
                not_indices.append(np.arange(
                    selected_locations[1] - Dict['startOfEpoch'], selected_locations[1] - Dict['startOfEpoch'] + selected_locations[3] - 1))
            else:
                for i in range(selected_locations.shape[0]):
                    data.append(Dict['data'][(selected_locations[i, 1] - Dict['startOfEpoch']): (
                        selected_locations[i, 1] - Dict['startOfEpoch']) + selected_locations[i, 3], :])
                    not_indices.append(np.arange(
                        selected_locations[i, 1] - Dict['startOfEpoch'], selected_locations[i, 1] - Dict['startOfEpoch'] + selected_locations[i, 3] - 1))
