...
class activation:
    ...
    def mean_fire(self, file_nr, sr, fr_zeros):
        ...
        if  hasattr(data_artifacts, '__len__') and hasattr(hypno_artifacts, '__len__'):
            ...
            length_sws_without_artifact = len([a for a in range(data.shape[0]) if (hypno[a] == -2 and hypno_artifacts[a] == 0)])
            length_rem_without_artifact = len([a for a in range(data.shape[0]) if (hypno[a] == -3 and hypno_artifacts[a] == 0)])
            length_awake_without_artifact = len([a for a in range(data.shape[0]) if (hypno[a] == 0 and hypno_artifacts[a] == 0)])
            ...
            length_sws_without_artifact = len([a for a in range(data.shape[0]) if (hypno[a] == -2)])
            length_rem_without_artifact = len([a for a in range(data.shape[0]) if (hypno[a] == -3)])
            length_awake_without_artifact = len([a for a in range(data.shape[0]) if (hypno[a] == 0)])
        ...
        if not(np.isnan(np.where(hypno == -2 , 1, np.nan)).all()):
            ...
            mean_fr_sws = np.sum(np.where(data_sws > 0 , 1, 0), axis = 0) / (length_sws_without_artifact/sr)
        ...
        if not(np.isnan(np.where(hypno == -3 , 1, np.nan)).all()):
            ...
            mean_fr_rem = np.sum(np.where(data_rem > 0 , 1, 0), axis = 0) / (length_rem_without_artifact/sr)
        ...
        if not(np.isnan(np.where(hypno == 0 , 1, np.nan)).all()):
            ...
            mean_fr_awake = np.sum(np.where(data_awake > 0 , 1, 0), axis = 0) / (length_awake_without_artifact/sr)
...
# initializing ETL
    ...
    if not(set_artifacts_):
        ...
        activation_ = activation(hypno=np.hstack(Hypno_.T)[:,np.newaxis], data=np.hstack(full_dataset.T).T[:,:,np.newaxis],
                             hypno_artifacts=np.hstack(hypno_artifacts.T)[:,np.newaxis],
...
def findseq(x):
    ...
    if (diff_ == 0).all():
        seq = np.zeros((1,4)).astype(np.int32)
        seq[0,0] = t0[0]
        seq[0,1] = 0
        seq[0,2] = len(t0) + 1
        seq[0,3] = len(t0) + 1
...
def select_subpopulation(avg_data, type_, percentage_):
    ...
    # finding nans locations after sorting data
    nan_locs = np.where(np.isnan(avg_data))[0]

    # printing information that nans will be removed before selecting xxx percentage
    if np.isnan(avg_data[index]).any():
        print(f' {len(nan_locs)} Nans are found in data and will be removed before selecting {percentage_}% of data')
        # removing nans from index_
        for i in range(len(nan_locs)):
            index = np.delete(index, np.where(index==nan_locs[i])[0])
...
class slo_spi:
    def __init__(self, file_address, data_location, channel_nr, start_index, \
                 end_index, start_end_file_add, file_length, elec_sample, \
                 unit_length, ca_fs, coupled):
        ...
        self.start_end_file_add = start_end_file_add
        self.file_length = file_length
        self.elec_sample = elec_sample
        self.unit_length = unit_length
        self.ca_fs = ca_fs
        self.coupled = coupled

    def load_mat_files(self):
        # this custom .mat reader use h5 format to read matlab nr_files
        # the hierarchy of matlab file is known by user and will be given to get location of variable_location

        import h5py

        # reading file_
        f = h5py.File(self.file_address)
        location_ = f[self.data_location][self.channel_nr[0], self.channel_nr[1]]
        # data refers to slo_spi location index
        full_data = np.array(f[location_])

        if self.coupled['apply']:
            coup_loc = f[self.coupled['address']][self.coupled['channel'][0], self.coupled['channel'][1]]
            coup_loc_value = np.array(f[coup_loc], dtype = np.int16)
            if self.coupled['cp']:
                self.data = full_data[np.hstack(coup_loc_value-1)] # python starts from 0
            else:
                temp = []
                temp = np.arange(0, len(full_data))
                temp_clean = np.delete(temp, np.hstack(coup_loc_value-1), axis = 0)
                self.data = full_data[temp_clean]
        else:
            self.data = full_data
        #return data

    def binary_mat_file(self):
        ...
        index_ = np.ones(self.file_length, dtype=np.int64)

        for i in range(data.shape[0]):
            index_[data[i,self.start_index]:data[i,self.end_index]]=0


        self.index_ = index_
        #return index_[start_cut:end_cut]

    def slo_spi_list(self):

        import pandas as pd
        from scipy import interpolate

        # reading start and end of the units
        unit_locs = pd.read_csv(self.start_end_file_add, sep=';')
        unit_locs = unit_locs.to_numpy()

        #producing binary index_
        self.binary_mat_file()
        # loading index_
        index_ = self.index_

        slo_spi = []
        for i in range(unit_locs.shape[0]):
            temp = []; temp_resample = []
            temp = index_[int(float(unit_locs[i,1])*self.elec_sample):int(float(unit_locs[i,2])*self.elec_sample)]

            # reampling temp to same size as hypno (fs*600 unit_length)
            f = interpolate.interp1d(np.linspace(0,temp.shape[0], temp.shape[0]), temp, kind = 'nearest')
            temp_resample = f(np.linspace(0, temp.shape[0], int(self.ca_fs*self.unit_length)))


            if (np.sum(temp_resample) == len(temp_resample)): # if all trace is 1

                slo_spi.append([0, len(temp_resample), int(i)])
            else:
                # finding sequences
                seq = []
                seq = findseq(temp_resample)
                seq = seq[seq['state'] == 1]
                seq = seq.to_numpy()

                # create list of artifact style

                for ii in range(seq.shape[0]):
                    slo_spi.append([seq[ii,1], seq[ii,2], int(i)])
